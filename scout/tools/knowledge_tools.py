...
def create_knowledge_tools():
            ...
            if row:
                ...
                return {
                    ...
                    "data": {
                        ...
                        "status": row[2],
                        "company_type": row[3],
                        "principal_activity": row[4],
                        "registered_office": row[5],
                        ...
                        "holding_company": row[11],
                        "foreign_company": row[12],
                    },
                }
    ...
    def get_directors(company_name: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "SELECT company_name_english, directors FROM companies WHERE company_name_english ILIKE %s LIMIT 1",
                (f"%{company_name}%",),
            )
            row = cur.fetchone()
            cur.close()
    ...
    def get_shareholders(company_name: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "SELECT company_name_english, members FROM companies WHERE company_name_english ILIKE %s LIMIT 1",
                (f"%{company_name}%",),
            )
            row = cur.fetchone()
            cur.close()
